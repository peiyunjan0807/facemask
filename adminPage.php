  ...
  <body>
    ...
    <h3>> 訂單 </h3>
      <div class="row">
        ...
        <?php
          // $x = $_SESSION['Email'];
          $query_user = ("select * from `user` inner join `order_table` on  `user`.`UserID` = `order_table`.`UserID` ;");
          ...
          for($i=0; $i<count($result); $i++){
            ...
            "<div class='card order-card' id='$y'>           
              <div class='card-body'>
                <span style='float:right; border:1px;'><button class='btn' type='submit' onClick=deleteOrder('$y')><img src='./img/close.png' style='width: 15px;'></button></span>
            </br>
              ...
              switch($state) {
                ...
                echo"<p class='status'>$state&emsp;&emsp;&emsp;&emsp;</p>
                <div class='d-flex justify-content-center'>
                ";
                if($state=="處理中"){
                  echo "<button class='btn btn-primary' type='submit'onClick=verifyOrder($y) style=''>接受訂單</button>";
                }
                echo"
              </div>
                </div></div>";
    ...
    <script> 
      ...
      function verifyOrder(order_id){
        location.href ="changeOrder.php?id="+order_id+"&action=1";
      }
      function deleteOrder(order_id){
        location.href ="changeOrder.php?id="+order_id+"&action=0";
      }
      </script>
  </body>
</html>
